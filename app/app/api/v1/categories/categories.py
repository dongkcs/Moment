...
@router.get("/list", summary="查看分类列表",dependencies=[DependPermisson])
...
@router.get("/get/alias", summary="查看分类")
async def get_category(
...
@router.post("/create", summary="创建分类",dependencies=[DependPermisson])
...
@router.post("/update", summary="更新分类",dependencies=[DependPermisson])
async def update_category(
    category_in: CategoryUpdate,
):
    await category_controller.update(id=category_in.id, obj_in=category_in.update_dict())
    return Success(msg="Updated Success")


@router.delete("/delete", summary="删除分类",dependencies=[DependPermisson])
