    ...
    <div class="inner split">
      <div class="inner split">
        <div>
          ...
          <section v-if="entries.length > 0" style="margin-bottom: 1.8rem">
            <h2>联系我</h2>
          ...
          <span style="color: #b5b5b5; font-size: 0.8em">
            <p v-if="isValueNotEmpty(icp)" class="footer-icp">
              ICP备案号:
              <a href="http://beian.miit.gov.cn/" target="_blank" rel="noopener nofollow">{{
                icp
              }}</a>
            </p>
          </span>
...
const primary_color = isValueNotEmpty(settingStore.metaSetting?.primary_color)
  ? settingStore.metaSetting?.primary_color
  : import.meta.env.VITE_PRIMARY_COLOR
const entries = isValueNotEmpty(settingStore.metaSetting?.entries)
  ? settingStore.metaSetting?.entries
  : []
const bottom_icon = isValueNotEmpty(settingStore.metaSetting?.bottom_icon)
  ? settingStore.metaSetting?.bottom_icon
  : import.meta.env.VITE_ICON
const bottom_desc = isValueNotEmpty(settingStore.metaSetting?.bottom_desc)
