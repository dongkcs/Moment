...
const columns = [
  ...
  { title: '时间', key: 'time', width: 100, ellipsis: { tooltip: true } },
  {
    ...
    width: 120,
    render(row) {
      return row.formatted_categories.map((e) =>
        h(
          NButton,
          {
            type: 'info',
  ...
  <CommonPage show-footer :title="$t('views.content.label_blog_list')">
        ...
        >
          <div flex style="width: 100%; flex-wrap: nowrap; flex-direction: row; column-gap: 10px">
            <n-popover trigger="hover" placement="bottom" :keep-alive-on-hover="false">
              ...
              <NImage
                v-if="modalForm.image != undefined && modalForm.image != ''"
                width="200"
                :src="modalForm.image + thumbnail_suffix"
                style="border-radius: 8px"
                show-toolbar-tooltip
                fallback-src="/images/error.svg"
              >
              </NImage>
    ...
    $t('views.content.label_get_picture_time') }}
        ...
        <NFormItem label="分类" path="categories">
          <NTreeSelect
            v-model:value="modalForm.categories"
            multiple
            checkable
            key-field="id"
            label-field="name"
